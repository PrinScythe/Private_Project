...
public class AnimationBilles implements Runnable {
	...
	private Vector<Bille> billes; // la liste de toutes les billes en mouvement
	private VueBillard vueBillard; // la vue responsable du dessin des billes
		...
		try {
			...
			while (!Thread.interrupted()) // gestion du mouvement
			{
				...
				for (i = 0; i < billes.size(); ++i) // mise � jour de la liste des billes
				{
					...
					billeCourante.déplacer(deltaT); // mise � jour position et vitesse de cette bille
					billeCourante.gestionAccélération(billes); // calcul de l'acc�l�ration subie par cette bille
				...
				Thread.sleep((int) deltaT); // deltaT peut �tre consid�r� comme le d�lai entre 2 flashes d'un
											// stroboscope qui �clairerait la sc�ne
			}
		}
